...
namespace MQHelper
{
    ...
    public class MQConnection
    {
        ...
        public IConnection Connection
        {
            get
            {
                if (connection == null)
                {
                    ...
                    cf.HostName = this.host;
                    cf.VirtualHost = this.vhost;
                    cf.Port = this.port;
        ...
        /// <returns></returns>
        public void Publish(IModel model, string exchange, string routeKey, string message)
        {
            if (string.IsNullOrEmpty(message)) return;
            byte[] data = Encoding.UTF8.GetBytes(message);
            model.BasicPublish(exchange, routeKey, null, data);
        }
