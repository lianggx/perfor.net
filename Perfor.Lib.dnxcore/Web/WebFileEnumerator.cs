...
namespace Perfor.Lib.Web
{
    ...
    public class WebFileEnumerator
    {
        ...
        public WebFileEnumerator(HttpRequest req,HttpResponse res)
        {
            request = req;
            response = res;
        }
        ...
        public void GetFileInfo(string filepath, string outputPage)
        {
            ...
            filePah = filepath;
            WebParams wp = new WebParams(request);
            ...
            if (isfile == false)
            {
                ...
                {
                    sourcePath =WebUtility.UrlEncode(sourcePath.Substring(0, sourcePath.LastIndexOf("/")));
                ...
                {
                    string div = string.Format("<div><a href='{0}?path={1}&isfile={2}&level={3}'>{4}</a><div/>", outputPage, WebUtility.UrlEncode(item.Path), item.IsFile.ToInt(), item.Level, item.Path);
                    WriteInfo(div);
                }
            }
            else {
